...
my_plot_2 <- ggplot(dat, aes(x = CPI, y = HDI, color = HDI)) + 
    geom_point()
my_plot_2 + scale_color_gradient(low = "darkviolet", high = "pink")
```
...
my_plot_2 + scale_color_viridis(option = "B")
```
...
1. Modify the HDI vs. CDI plot to use a different color and symbol for each region.
...
2. Overlay a smoothling line on top of the scatter plot using `geom_line`. Hint: change the statistical transformation.
...
* https://github.com/butterflyology/spaceMovie
