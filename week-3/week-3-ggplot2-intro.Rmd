...
## 1. Scatter plot with CPI on the x axis and HDI on the y axis
...
## 3. Color according to region

```{r}
p + geom_point(aes(col=Region))
```

## 4. Boxplots of CPI by region

```{r}
ggplot(dat, aes(x=Region,y=CPI)) + geom_boxplot()
```

## 5. Overlay points on top of boxplots and save to file
